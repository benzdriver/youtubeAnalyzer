        ...
        "segments": [
            {
                "start": 0,
                "end": 5,
                "text": "This is a test video about machine learning.",
                "confidence": 0.95,
            },
            {
                "start": 5,
                "end": 10,
                "text": "We will cover neural networks and deep learning concepts.",
                "confidence": 0.92,
            },
        ],
    }
...
async def test_analyze_content_success(
    analyzer, sample_transcript_data, sample_video_info
):
...
async def test_extract_key_points(analyzer, sample_transcript_data):
    ...
    with patch.object(analyzer, "_make_api_call", return_value=mock_response):
        result = await analyzer._extract_key_points(
            sample_transcript_data["full_text"], sample_transcript_data["segments"]
        )
...
async def test_calculate_quality_score(analyzer):
    ...
    score = analyzer._calculate_quality_score(
        key_points, topic_analysis, sentiment_analysis, 1000
    )
