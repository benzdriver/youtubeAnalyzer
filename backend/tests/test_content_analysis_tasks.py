...
from app.services.content_analyzer import ContentInsights, KeyPoint, TopicAnalysis, SentimentAnalysis, ContentStructure, ContentType, SentimentType
...
def sample_transcript_data():
    ...
    return {
        ...
        'segments': [
            {'start': 0, 'end': 10, 'text': 'This is a comprehensive tutorial', 'confidence': 0.95},
            {'start': 10, 'end': 20, 'text': 'on machine learning algorithms.', 'confidence': 0.93}
        ]
    }
...
def sample_content_insights():
    ...
    return ContentInsights(
        ...
        sentiment_analysis=SentimentAnalysis(
            ...
            sentiment_progression=[{"timestamp": 0, "sentiment": "positive", "score": 0.4}]
        ),
        ...
        recommendations=["Practice with real datasets", "Implement algorithms from scratch"],
        quality_score=0.82
    )


@pytest.mark.asyncio
async def test_analyze_content_task_success(sample_transcript_data, sample_video_info, sample_content_insights):
    """Test successful content analysis task execution"""
    task_id = "test-task-123"
    
    with patch('app.tasks.content_analysis.AsyncSessionLocal') as mock_session_local, \
        ...
        result = await analyze_content_task(task_id, sample_transcript_data, sample_video_info)
        
        mock_analyzer.analyze.assert_called_once_with(sample_transcript_data, sample_video_info)
...
async def test_content_insights_serialization(sample_content_insights):
    ...
    with patch('app.tasks.content_analysis.AsyncSessionLocal') as mock_session_local, \
        ...
        result = await analyze_content_task(task_id, {'full_text': 'test', 'segments': []}, {'title': 'test'})
...
async def test_analyzer_failure_handling(sample_transcript_data, sample_video_info):
    ...
    with patch('app.tasks.content_analysis.AsyncSessionLocal') as mock_session_local, \
        ...
        with pytest.raises(ExternalServiceError):
            await analyze_content_task(task_id, sample_transcript_data, sample_video_info)
