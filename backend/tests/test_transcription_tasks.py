...
def mock_transcript_result():
    ...
    return TranscriptResult(
        ...
        segments=[
            ...
            TranscriptSegment(start=5.0, end=10.0, text='This is a test', confidence=-0.3)
        ],
...
@pytest.mark.asyncio
async def test_transcribe_audio_task_success(mock_transcript_result, mock_db_session, mock_analysis_task):
    ...
    with patch('app.tasks.transcription.AsyncSessionLocal') as mock_session_local:
        ...
        with patch('app.tasks.transcription.TranscriptionService') as mock_service_class:
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            
            mock_service.validate_audio_file.return_value = True
            mock_service.detect_language = AsyncMock(return_value={
                'language': 'en',
                'confidence': 0.95
            })
            mock_service.transcribe_audio = AsyncMock(return_value=mock_transcript_result)
            
            with patch('app.tasks.transcription.send_progress_update') as mock_progress:
                
                result = await transcribe_audio_task("test-task-id", "/path/to/audio.wav")
                
                mock_service.validate_audio_file.assert_called_once_with("/path/to/audio.wav")
                mock_service.detect_language.assert_called_once_with("/path/to/audio.wav")
                mock_service.transcribe_audio.assert_called_once_with("/path/to/audio.wav", language='en')
...
@pytest.mark.asyncio
async def test_transcribe_audio_task_with_provided_language(mock_transcript_result, mock_db_session, mock_analysis_task):
    ...
    with patch('app.tasks.transcription.AsyncSessionLocal') as mock_session_local:
        ...
        with patch('app.tasks.transcription.TranscriptionService') as mock_service_class:
            ...
            with patch('app.tasks.transcription.send_progress_update'):
                ...
                result = await transcribe_audio_task("test-task-id", "/path/to/audio.wav", language="zh")
                
                assert not hasattr(mock_service, 'detect_language') or not mock_service.detect_language.called
                mock_service.transcribe_audio.assert_called_once_with("/path/to/audio.wav", language='zh')


@pytest.mark.asyncio
async def test_transcribe_audio_task_low_language_confidence(mock_transcript_result, mock_db_session, mock_analysis_task):
    ...
    with patch('app.tasks.transcription.AsyncSessionLocal') as mock_session_local:
        ...
        with patch('app.tasks.transcription.TranscriptionService') as mock_service_class:
            ...
            with patch('app.tasks.transcription.send_progress_update'):
                ...
                mock_service.transcribe_audio.assert_called_once_with("/path/to/audio.wav", language=None)
...
@pytest.mark.asyncio
async def test_transcribe_audio_task_invalid_audio_file(mock_db_session, mock_analysis_task):
...
@pytest.mark.asyncio
async def test_transcribe_audio_task_transcription_failure(mock_db_session, mock_analysis_task):
    ...
    with patch('app.tasks.transcription.AsyncSessionLocal') as mock_session_local:
        ...
        with patch('app.tasks.transcription.TranscriptionService') as mock_service_class:
            ...
            mock_service.transcribe_audio = AsyncMock(side_effect=ExternalServiceError("Transcription failed"))
...
@pytest.mark.asyncio
async def test_transcribe_audio_task_database_update(mock_transcript_result, mock_db_session, mock_analysis_task):
    ...
    with patch('app.tasks.transcription.AsyncSessionLocal') as mock_session_local:
        ...
        with patch('app.tasks.transcription.TranscriptionService') as mock_service_class:
            ...
            with patch('app.tasks.transcription.send_progress_update'):
                ...
                assert mock_db_session.execute.call_count >= 2  # At least select and update
                mock_db_session.commit.assert_called()


@pytest.mark.asyncio
async def test_transcribe_audio_task_progress_updates(mock_transcript_result, mock_db_session, mock_analysis_task):
    ...
    with patch('app.tasks.transcription.AsyncSessionLocal') as mock_session_local:
        ...
        with patch('app.tasks.transcription.TranscriptionService') as mock_service_class:
            ...
            with patch('app.tasks.transcription.send_progress_update') as mock_progress:
                ...
                progress_values = [call[0][1] for call in progress_calls]  # Second argument is progress
