        ...
        try:
            ...
            if e.resp.status == 403:
                if 'quotaExceeded' in str(e):
                    raise RateLimitError(f"YouTube API配额已用完", service="youtube_api", retry_after=3600)
                elif 'forbidden' in str(e).lower():
                    raise YouTubeAPIError(f"视频访问被禁止: {video_id}", error_code="forbidden")
            elif e.resp.status == 404:
                raise YouTubeAPIError(f"视频不存在: {video_id}", error_code="not_found")
            elif e.resp.status == 429:
                raise RateLimitError(f"YouTube API请求过于频繁", service="youtube_api", retry_after=300)
            else:
                raise YouTubeAPIError(f"YouTube API调用失败: {e}", error_code=str(e.resp.status))
            ...
            while len(comments) < max_results:
                ...
                for item in response['items']:
                    ...
                    comment_data = CommentData(
                        ...
                        is_author_reply=top_comment.get('authorChannelId', {}).get('value', '') == video_channel_id
                    )
                    ...
                    if 'replies' in item:
                        for reply in item['replies']['comments']:
                            ...
                            reply_data = CommentData(
                                ...
                                is_author_reply=reply_snippet.get('authorChannelId', {}).get('value', '') == video_channel_id,
