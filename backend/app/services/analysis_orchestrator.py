...
class AnalysisOrchestrator:
    ...
    def _calculate_progress(self, completed_steps: List[AnalysisStep], current_step: AnalysisStep, step_progress: int = 50) -> float:
        """计算总体进度百分比"""
        total_progress = 0.0
    ...
    async def _update_task_status(self, task_id: str, status: TaskStatus, current_step: Optional[str] = None, completed_at: Optional[datetime] = None):
        """更新任务状态"""
        update_data = {
            "status": status,
    ...
    async def _run_extraction_step(self, task_id: str, youtube_url: str, options: Dict[str, Any]) -> StepResult:
        """执行YouTube数据提取步骤"""
        ...
        try:
            ...
            await send_progress_update(task_id, 5, "初始化YouTube提取器", AnalysisStep.EXTRACTION)
    ...
    async def _run_transcription_step(self, task_id: str, extraction_data: Optional[Dict[str, Any]], options: Dict[str, Any]) -> StepResult:
        """执行音频转录步骤"""
        step_start = datetime.utcnow()
        ...
        try:
            ...
            await send_progress_update(task_id, 30, "开始音频转录", AnalysisStep.TRANSCRIPTION)
            
            transcription_result = await transcribe_audio_task(task_id, audio_file_path, language)
            
            duration = (datetime.utcnow() - step_start).total_seconds()
            
            await send_progress_update(task_id, 55, "音频转录完成", AnalysisStep.TRANSCRIPTION)
    ...
    async def _run_content_analysis(self, task_id: str, extraction_data: Optional[Dict[str, Any]], transcription_data: Optional[Dict[str, Any]], options: Dict[str, Any]) -> StepResult:
        """执行内容分析步骤"""
        step_start = datetime.utcnow()
        ...
        try:
            ...
            content_result = await analyze_content_task(task_id, transcription_data, video_info)
    ...
    async def _run_comment_analysis(self, task_id: str, extraction_data: Optional[Dict[str, Any]], options: Dict[str, Any]) -> StepResult:
        """执行评论分析步骤"""
        step_start = datetime.utcnow()
        ...
        try:
            ...
            comment_result = await analyze_comments_task(task_id, video_id, comments_data)
    ...
    async def _run_parallel_analysis_steps(self, task_id: str, extraction_data: Optional[Dict[str, Any]], transcription_data: Optional[Dict[str, Any]], options: Dict[str, Any]) -> tuple:
        """并行执行内容分析和评论分析"""
    ...
    async def _run_finalization_step(self, task_id: str, step_results: Dict[AnalysisStep, StepResult], options: Dict[str, Any]) -> StepResult:
        """执行最终报告生成步骤"""
        step_start = datetime.utcnow()
        
        try:
            await send_progress_update(task_id, 90, "生成综合分析报告", AnalysisStep.FINALIZATION)
            ...
            extraction_data = extraction_result.data if extraction_result and extraction_result.data else {}
            transcription_data = transcription_result.data if transcription_result and transcription_result.data else {}
            content_data = content_result.data if content_result and content_result.data else {}
            comment_data = comment_result.data if comment_result and comment_result.data else {}
    ...
    async def _generate_comprehensive_report(self, extraction_data: Dict[str, Any], transcription_data: Dict[str, Any], 
                                           content_data: Dict[str, Any], comment_data: Dict[str, Any], 
        ...
        overall_score = self._calculate_overall_score(content_analysis, comment_analysis)
        ...
        comprehensive_report = {
            ...
            "transcript_analysis": {
                ...
                "speaking_rate_wpm": round(transcript_info["word_count"] / (transcript_info["duration"] / 60), 1) if transcript_info["duration"] else 0
            },
            ...
            "metadata": {
                ...
                "data_sources": ["youtube_api", "whisper_transcription", "openai_analysis"]
            }
        }
        
        return comprehensive_report
    
    def _generate_cross_analysis_insights(self, content_analysis: Dict[str, Any], comment_analysis: Dict[str, Any], 
                                        video_info: Dict[str, Any], transcript_info: Dict[str, Any]) -> Dict[str, Any]:
        """生成跨分析洞察"""
        ...
        content_sentiment = content_analysis.get("sentiment_analysis", {}).get("overall_sentiment")
        ...
        if content_sentiment and comment_sentiment:
            ...
            total_comments = sum(comment_sentiment.values()) if comment_sentiment.values() else 1
            ...
            insights["content_audience_alignment"] = {
                ...
                "alignment_score": self._calculate_alignment_score(content_sentiment, positive_ratio)
            }
        ...
        if view_count > 0:
            ...
            insights["engagement_patterns"] = {
                ...
                "engagement_quality": "high" if engagement_rate > 0.05 else "medium" if engagement_rate > 0.01 else "low"
            }
        
        content_topics = content_analysis.get("topic_analysis", {}).get("main_topics", [])
        comment_keywords = comment_analysis.get("key_themes", [])
        
        if content_topics and comment_keywords:
            topic_overlap = self._calculate_topic_overlap(content_topics, comment_keywords)
            insights["topic_resonance"] = {
                ...
                "resonance_level": "high" if topic_overlap > 0.6 else "medium" if topic_overlap > 0.3 else "low"
            }
        
        return insights
    
    def _generate_recommendations(self, content_analysis: Dict[str, Any], comment_analysis: Dict[str, Any], 
                                video_info: Dict[str, Any], transcript_info: Dict[str, Any]) -> Dict[str, Any]:
        """生成改进建议"""
        ...
        comment_sentiment = comment_analysis.get("sentiment_distribution", {})
        negative_ratio = comment_sentiment.get("negative", 0) / sum(comment_sentiment.values()) if comment_sentiment.values() else 0
        
        if negative_ratio > 0.3:
            recommendations["audience_engagement"].append("关注负面反馈，考虑在后续内容中回应观众关切")
        
        creator_responses = comment_analysis.get("creator_interaction", {}).get("response_rate", 0)
    ...
    def _calculate_alignment_score(self, content_sentiment: str, positive_ratio: float) -> float:
        """计算内容与观众情感对齐分数"""
    ...
    def _calculate_topic_overlap(self, content_topics: List[str], comment_keywords: List[str]) -> float:
        """计算内容话题与评论关键词的重叠度"""
    ...
    def _calculate_overall_score(self, content_analysis: Dict[str, Any], comment_analysis: Dict[str, Any]) -> float:
        """计算整体分析评分"""
        scores = []
        
        content_quality = content_analysis.get("quality_metrics", {}).get("overall_quality", 0.5)
        scores.append(content_quality)
        
        sentiment_dist = comment_analysis.get("sentiment_distribution", {})
        if sentiment_dist:
            positive_ratio = sentiment_dist.get("positive", 0) / sum(sentiment_dist.values())
            scores.append(positive_ratio)
        
        structure_score = content_analysis.get("content_structure", {}).get("overall_structure_score", 0.5)
