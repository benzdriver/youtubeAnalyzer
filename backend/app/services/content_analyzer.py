...
class LLMContentAnalyzer(BaseContentAnalyzer):
    ...
    async def analyze(self, transcript_data: Dict[str, Any], video_info: Dict[str, Any]) -> ContentInsights:
        ...
        try:
            ...
            logging.info(f"Starting content analysis for video: {video_info.get('title', 'Unknown')}")
            
            try:
                key_points, topic_analysis, sentiment_analysis, content_structure, summary, recommendations = await asyncio.gather(
    ...
    async def _extract_key_points(self, text: str, segments: List[Dict]) -> List[KeyPoint]:
        ...
        try:
            ...
            for kp_data in result.get('key_points', []):
                start_time, end_time = self._find_timestamp_for_text(kp_data['text'], segments)
    ...
    async def _analyze_topics(self, text: str, video_info: Dict[str, Any]) -> TopicAnalysis:
    ...
    async def _generate_recommendations(self, text: str, video_info: Dict[str, Any]) -> List[str]:
    ...
    async def _make_api_call(self, prompt: str, max_tokens: int = 500, temperature: float = 0.3) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise ExternalServiceError(f"OpenAI API call failed after {self.max_retries} attempts: {e}")
