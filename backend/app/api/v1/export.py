...
async def export_json(
    task_id: str, pretty: bool = Query(False, description="Pretty print JSON")
):
...
async def export_summary(task_id: str):
    ...
    try:
        ...
        if content_insights.get("key_points"):
            summary_lines.extend(["=== Key Points ===", ""])
            for i, point in enumerate(content_insights["key_points"][:10], 1):
                timestamp = point.get("timestamp", 0)
                minutes = int(timestamp // 60)
                seconds = int(timestamp % 60)
                summary_lines.append(
                    f"{i}. [{minutes:02d}:{seconds:02d}] {point.get('content', '')}"
                )
            summary_lines.append("")

        if content_insights.get("topics"):
            summary_lines.extend(
                ["=== Topics ===", ", ".join(content_insights["topics"]), ""]
            )
...
async def share_result(
    task_id: str,
    platform: str = Query(
        ..., description="Sharing platform (twitter, linkedin, etc.)"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Share generation failed: {str(e)}"
        )
